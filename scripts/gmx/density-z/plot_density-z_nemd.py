...
"""
Plot density profiles for a Non-Equilibrium Molecular Dynamics (NEMD)
simulation and its equilibrium counterpart(s) (EQMD).
"""
...
parser = argparse.ArgumentParser(
    description=(
        "Plot density profiles for a NEMD simulation and its EQMD"
        " counterpart(s)."
    ),
)
parser.add_argument(
...
if args.common_ylim:
    outfile = base_name_nemd + "_common_ylim.pdf"  # Output file name.
else:
    outfile = base_name_nemd + ".pdf"
...
if args.common_ylim:
    ...
    ymax = None
ymax = tuple(tuple(ymax for _cmp in compounds) for _plt_sec in plot_sections)
        ...
        for ps_ix, plt_sec in enumerate(plot_sections):
            for cmp_ix, cmp in enumerate(compounds):
                ...
                for sim_ix, infile in enumerate(infiles):
                    ...
                    if y_normed:
                        bulk_dens = Sims.sims[0].dens["atm_type"][cmp]["num"]
