...
def legend_title(pk_pos_type=None, direction=None):
    r"""
    Create a legend title string.

    Parameters
    ----------
    pk_pos_type : {None, "left", "right"}, optional
        Peak position type.  Whether the peak is in the left or right
    ...
    """
    if pk_pos_type not in (None, "left", "right"):
        raise ValueError("Unknown `pk_pos_type`: '{}'".format(pk_pos_type))
    if direction not in (None, "l2r", "r2l"):
        raise ValueError("Unknown `direction`: '{}'".format(direction))

    if pk_pos_type == "left":
        surfq_sign = "+"
    elif pk_pos_type == "right":
        surfq_sign = "-"
    else:
        surfq_sign = r"\pm"
    title = (
...
parser = argparse.ArgumentParser(
    description=(
        ...
        " given compound as function of the salt concentration."
    )
)
parser.add_argument(
    "--sol",
...
with PdfPages(outfile) as pdf:
    for col_ix in range(len(cols)):
        for pkt_ix, pkp_type in enumerate(pk_pos_types):
            for drctn_ix, drctn in enumerate(directions):
                ...
                equalize_yticks(ax)
                legend = ax.legend(
                    title=legend_title(pkp_type, drctn) + legend_title_suffix,
                    ncol=2,
    ...
    for pkt_ix, pkp_type in enumerate(pk_pos_types):
        ...
        for drctn_ix, drctn in enumerate(directions):
            ...
            legend = ax.legend(
                title=legend_title(pkp_type, drctn) + "s",
                loc="best",
                **mdtplt.LEGEND_KWARGS_XSMALL,
            )
            legend.get_title().set_multialignment("center")
