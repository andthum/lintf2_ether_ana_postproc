...
"""
Plot the time-averaged potential energy of the system divided by the
...
# Output filename.
outfile = "eq_npT_pr_nh_" + system + "_epot_per_chain.pdf"

# Columns to read from the input files.
cols = (
    0,  # Number of ether oxygens per PEO chain.
    1,  # Number of PEO chains.
    3,  # Potential energy [kJ].
    4,  # Standard deviation of the potential energy [kJ].
)
...
for set_ix, infile in enumerate(infiles):
    ...
    # Divide potential energy by number of PEO chain.
...
if args.sol in ("g1", "peo63"):
    fit_starts = (0,)
    fit_stops = (np.flatnonzero(np.isclose(xdata[0], 1 / 6))[0] + 1,)
elif args.sol == "g4":
    fit_starts = (0, np.flatnonzero(np.isclose(xdata[0], 1 / 5))[0])
    fit_stops = (
        np.flatnonzero(np.isclose(xdata[0], 1 / 6))[0] + 1,
        len(xdata[0]),
    )
else:
...
with PdfPages(outfile) as pdf:
    ...
    for set_ix, temp in enumerate(temps):
        ...
        for fit_ix, popt in enumerate(popts[set_ix]):
            ...
            y_offset = 0.03 * ydata_max_diff
            ax.text(
                x_pos,
                y_pos - y_offset if set_ix == 0 else y_pos + y_offset,
                r"$%.3f r %+.3f$" % tuple(popt),
    ...
    ax.set(xlabel=xlabel, ylabel=r"$E_{pot} / N_{Chain}$ / kJ", xlim=xlim_lin)
    equalize_xticks(ax)
