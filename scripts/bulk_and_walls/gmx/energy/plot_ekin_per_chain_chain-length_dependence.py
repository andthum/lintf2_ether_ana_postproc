...
"""
Plot the time-averaged kinetic energy of the system divided by the
...
# Output filename.
outfile = "eq_npT_pr_nh_" + system + "_ekin_per_chain.pdf"
...
cols = (
    ...
    5,  # Kinetic energy [kJ].
    6,  # Standard deviation of the kinetic energy [kJ].
)
...
for set_ix, infile in enumerate(infiles):
    ...
    # Divide kinetic energy by number of PEO chain.
...
with PdfPages(outfile) as pdf:
    ...
    ax.set(xlabel=xlabel, ylabel=r"$E_{kin} / N_{Chain}$ / kJ", xlim=xlim_lin)
    ...
    ax.relim()
    ax.autoscale()
    ax.set(xlim=(1.5, 8.5), ylim=(40, 310))
