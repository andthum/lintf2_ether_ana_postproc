...
as function of the electrode surface charge.
...
outfile = (  # Output file name.
    ...
    + "_lintf2_"
    + args.sol
    + "_20-1_gra_qX_sc80_free-energy_trans_rate_"
    + args.cmp
...
if args.common_ylim:
    if args.cmp == "Li":
        ylims = [
            ...
            (8e-1, 6e4),  # (9e-1, 7e3),  # 9 Inverse transition rates, l2r.
            (8e-1, 6e3),  # (9e-1, 6e2),  # 10 Inverse transition rates, r2l.
            (8e-1, 6e4),  # (9e-1, 7e3),  # 11 Inverse transition rates, mean.
        ]
    else:
    ...
    for col_ix, yd_col in enumerate(ydata):
        ...
        for pkt_ix, pkp_type in enumerate(pk_pos_types):
            ...
            for cix_pkt in clstr_ix_unq[pkt_ix]:
                ...
                ax_sep.plot(
                    xdata_fac * xdata[pkt_ix][valid][yd_is_finite],
            ...
            legend_sep = ax_sep.legend(
                title=legend_title(direction[col_ix]) + legend_title_suffix,
                ncol=2,
