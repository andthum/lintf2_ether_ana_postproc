...
bins for various surface charges.
"""
...
parser = argparse.ArgumentParser(
    description=(
        ...
        " selected bins for various surface charges."
    )
)
parser.add_argument(
    "--sol",
...
with PdfPages(outfile) as pdf:
    ...
    for sim_ix, hist in enumerate(hists_bulk):
        ax.stairs(
            ...
            hists_bins[sim_ix],
            fill=False,
            label=r"$\pm %.2f$" % Sims.surfqs[sim_ix],
    ...
    legend_title_pdf = legend_title + "Bulk Region" + legend_title_suffix
    legend = ax.legend(
        ...
        for cix_pkt in clstr_ix_unq[pkt_ix]:
            ...
            for six, sim_ix in enumerate(clstr_sim_indices):
                ...
                ax.stairs(
                    ...
                    hists_bins[sim_ix],
                    fill=False,
                    label=r"$" + surfq_sign + r"%.2f$" % Sims.surfqs[sim_ix],
                    color=colors[sim_ix],
                    alpha=leap.plot.ALPHA,
                    rasterized=False,
                )
    ...
    for pkt_ix, pkp_type in enumerate(pk_pos_types):
        ...
        for cix_pkt in clstr_ix_unq[pkt_ix]:
            ...
            clstr_position = np.mean(clstr_bin_mids)
            ax.scatter(
                xdata_fac * xdata[pkt_ix][valid_clstr],
                clstr_bin_mids,
                color=cmap(cmap_norm(cix_pkt)),
                marker=markers[pkt_ix],
                label=r"$\sim %.2f$" % clstr_position,
            )
        ax.set(xlabel=xlabel, ylabel=ylabel, xlim=xdata_fac * xlim, ylim=ylim)
        equalize_xticks(ax)
        legend = ax.legend(
