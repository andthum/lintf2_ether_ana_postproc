...
"""
Plot the number of renewal events in each bin as function of the salt
concentration.
...
conc, rt_bulk = np.loadtxt(fpath_rt_bulk, usecols=cols_rt_bulk, unpack=True)
if not np.all(conc[:-1] <= conc[1:]):
    raise ValueError(
        "The bulk simulations are not sorted by the salt concentration"
    )
if not np.all(Sims.Li_O_ratios[:-1] <= Sims.Li_O_ratios[1:]):
    raise ValueError(
        "The surface simulations are not sorted by the salt concentration"
    )
dec = 4
if not np.all(np.isin(np.round(Sims.Li_O_ratios, dec), np.round(conc, dec))):
    raise ValueError(
        "The salt concentrations in the surface simulations ({}) do not match"
        " to those in the bulk simulations ({})".format(Sims.Li_O_ratios, conc)
    )
valid_bulk_sim = np.isin(np.round(conc, dec), np.round(Sims.Li_O_ratios, dec))
conc, rt_bulk = conc[valid_bulk_sim], rt_bulk[valid_bulk_sim]
...
for sim_ix, Sim in enumerate(Sims.sims):
    ...
    try:
        ...
        # In the system lintf2_g4_5-1_sc80 no renewal events have been
        # detected.
        n_events_bulk_sim = 0
    else:
        n_events_bulk_sim = len(
            np.loadtxt(infile_ri_bulk, usecols=cols_ri_bulk)
        )
...
if args.common_ylim:
    ylims = [
        ...
        (-2, 50),  # N_cmp^layer.
        (-0.02, 0.48),  # N_cmp^layer / N_cmp^tot.
        (6e-1, 7e4),  # N_events^layer.
        (5e-2, 4e3),  # N_events^layer / N_cmp^layer.
        (4e-3, 3e1),  # layer / bulk.
        (3e-2, 3e2),  # bulk / layer.
        (3e-1, 9e3),  # tau_3^layer.
    ]
else:
