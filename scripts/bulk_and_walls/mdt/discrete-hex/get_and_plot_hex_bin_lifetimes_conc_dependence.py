...
The residence time is calculated directly from the discrete
...
parser.add_argument(
    ...
    default=0.01,
    help=(
        "Only calculate the residence time by directly integrating the remain"
...
slab_starts = np.full(Sims.n_sims, np.nan, dtype=np.float32)
...
for sim_ix, path in enumerate(Sims.paths_ana):
    ...
    slab_starts_sim = np.full(len(dirs), np.nan, dtype=np.float32)
    slab_stops_sim = np.full_like(slab_starts_sim, np.nan)
    for d_ix, directory in enumerate(dirs):
        slab_starts_sim[d_ix], slab_stops_sim[d_ix] = get_slab(
            directory, dir_prefix
        )
    ix_max = np.argmax(slab_stops_sim)
    ana_dirs[sim_ix] = dirs[ix_max]
    slab_starts[sim_ix] = slab_starts_sim[ix_max]
    slab_stops[sim_ix] = slab_stops_sim[ix_max]
del dirs, slab_starts_sim, slab_stops_sim
...
for sim_ix, Sim in enumerate(Sims.sims):
    ...
    ana_dir = ana_dirs[sim_ix]
    slab_str = "_%.2f-%.2fA" % (slab_starts[sim_ix], slab_stops[sim_ix])
    file_suffix = analysis + analysis_suffix + slab_str + "_dtrj.npz"
    infile = os.path.join(ana_dir, Sim.fname_ana_base + file_suffix)
    ...
    # residence time is calculated as the inverse transition rate.
    rate = mdt.dtrj.trans_rate_tot(dtrj, discard_all_neg=True)
    lts_k[sim_ix] = time_conv / rate
del dtrj, rate


print("Calculating residence times from the remain probability...")
times_prev = None
...
for sim_ix, Sim in enumerate(Sims.sims):
    ...
    file_suffix = (
        ...
        + analysis_suffix
        + slab_str
        + "_state_lifetime_discard-all-neg"
        + con
        + ".txt.gz"
    )
    infile = os.path.join(ana_dir, Sim.fname_ana_base + file_suffix)
...
print("Calculating residence times from the Kaplan-Meier estimator...")
times_prev = None
...
for sim_ix, Sim in enumerate(Sims.sims):
    ...
    file_suffix = (
        ...
        + analysis_suffix
        + slab_str
        + "_kaplan_meier_discard-all-neg.txt.gz"
    )
    infile = os.path.join(ana_dir, Sim.fname_ana_base + file_suffix)
...
header = (
    "Residence time on the hexagonal lattice sites on the electrode surface.\n"
    + "\n"
    + "Compound:        {}\n".format(args.cmp)
    ...
    + "\n"
    + "The columns contain:\n"
    + "  1 Ratio of lithium ions to ether oxygens\n"
    + "\n"
    ...
    + "\n"
    + "  Method 3: Inverse transition rate\n"
    + " 30 Mean residence time / ns\n"
    + "\n"
    + "Methods based on the ACF:\n"
    + "  Method 4: Numerical integration of the ACF\n"
    + " 31 Mean residence time (1st raw moment) / ns\n"
