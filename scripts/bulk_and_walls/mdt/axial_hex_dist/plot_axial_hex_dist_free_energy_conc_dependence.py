...
data = np.column_stack([Sims.Li_O_ratios, barriers, barriers_sd])
...
with PdfPages(outfile_pdf) as pdf:
    ...
    fig, ax = plt.subplots(clear=True)
    ax.plot(Sims.Li_O_ratios, n_peaks, marker="o")
    ax.set(xlabel=xlabel, ylabel=ylabel, xlim=xlim)
    equalize_xticks(ax)
    equalize_yticks(ax)
    legend = ax.legend(title=legend_title.split("\n")[0].replace(", ", "\n"))
    ...
    fig, ax = plt.subplots(clear=True)
    ax.errorbar(Sims.Li_O_ratios, barriers, yerr=barriers_sd, marker="o")
    ax.set(xlabel=xlabel, ylabel=ylabel, xlim=xlim)
    equalize_xticks(ax)
    equalize_yticks(ax)
    legend = ax.legend(title=legend_title.split("\n")[0].replace(", ", "\n"))
    ...
    # xlim_full = (0, 19 * r0)
    xlim = (0, 6 * r0)
    ylim = (-1.8, 1.2)  # If free-energy maxima are set to zero.
