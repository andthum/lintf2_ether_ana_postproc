...
information() {
    ...
    echo "Python script plot_densmap-z.py to plot 2-dimensional density"
    echo "maps."
}
...
py_script=$(readlink -e "${script_dir}/plot_densmap-z.py" || exit)
...
for surfq in q[0-9]*; do
    ...
    for system in lintf2_[gp]*[0-9]*_[0-9]*-[0-9]*_gra_"${surfq}"_sc80; do
        ...
        for cmp_dir in "${analysis}_"*; do
            echo
            echo "${cmp_dir}"
            if [[ ! -d ${cmp_dir} ]]; then
                echo "WARNING: No such directory: '${cmp_dir}'"
                continue
            fi
            cmp="${cmp_dir#"${analysis}"_}"
            cd "${cmp_dir}" || exit

            ana_files="${settings}_${system}_${analysis}_${cmp}"
            ana_files_suffix=".txt.gz"
            for ana_file in "${ana_files}"*"${ana_files_suffix}"; do
                if [[ ! -f ${ana_file} ]]; then
                    echo "WARNING: No such file: '${ana_file}'"
                    continue
                fi
                slab="${ana_file#"${ana_files}"_}"
                slab="${slab%"${ana_files_suffix}"}"
